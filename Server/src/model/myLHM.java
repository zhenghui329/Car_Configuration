...
//        }
    ...
    public java.lang.String getAllModels(){
        ...
        while(itr.hasNext()){
            ...
            counter++;
            if(t != null){
                allModels.append(counter + " " + t.getMake()+ " " + t.getName() + " " + t.getYear() + '\n');
            }
        }
        return allModels.toString();
    }

    public java.lang.String getModelList(){
        StringBuilder allModels = new StringBuilder();
        Collection<T> c = lHashMap.values();
        Iterator<T> itr = c.iterator();
        while(itr.hasNext()){
    ...
    public Automobile getModel(int n) {
        if (n > 0 && n <= lHashMap.size()) {
            Collection<T> c = lHashMap.values();
            Iterator<T> itr = c.iterator();
            T t = null;
            for (int i = 0; i < n; i++) {
